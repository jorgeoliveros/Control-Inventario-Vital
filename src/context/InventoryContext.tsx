...
export const InventoryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const registerStockEntry = (entryData: {
    ...
    const product = products.find(p => String(p.id) === String(entryData.productId) || p.sku === String(entryData.productId));
    ...
    setProducts(prev =>
      prev.map(p => {
        if (String(p.id) === String(product.id) || p.sku === product.sku) {
          return {
            ...p,
  ...
  const registerStockExit = (exitData: {
    ...
    const product = products.find(p => String(p.id) === String(exitData.productId) || p.sku === String(exitData.productId));
    if (!product) return null;
